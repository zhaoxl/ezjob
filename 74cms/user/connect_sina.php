...
if($act == 'login' && empty($code))
{
	...
	$code_url = $o->getAuthorizeURL("http://{$_CFG['site_domain']}{$_CFG['site_dir']}user/connect_sina.php");
...
elseif($act == 'login' && !empty($code))
{
	...
	if (!empty($code))
	{
		$keys = array();
		$keys['code'] = $code;
		$keys['redirect_uri'] ="http://{$_CFG['site_domain']}{$_CFG['site_dir']}user/connect_sina.php";
		try {
	...
	{
	$link[0]['text'] = "返回上一页";
	$link[0]['href'] = "http://{$_CFG['site_domain']}{$_CFG['site_dir']}user/connect_sina.php";
