...
<div id="page-wrap">
  <div id="main-wrap">
	...
	<dl class="nav-left mt10">
        ...
        <?php if(is_array($tab_list)): ?><?php $i = 0;?><?php $__LIST__ = $tab_list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$tab): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dd <?php if( $tab['field_key'] == ACTION_NAME ): ?><?php $tabName = $tab['field_name']; ?> class="current"<?php endif; ?>><a href="<?php if($tab['url']): ?><?php echo ($tab["url"]); ?><?php else: ?><?php echo U('public/Account/'.$tab['field_key']);?><?php endif; ?>">
        <?php echo ($tab["field_name"]); ?>
        </a></dd><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
    </dl>
    <dl class="nav-left mt10">
		
        <?php if(is_array($tab_list_preference)): ?><?php $i = 0;?><?php $__LIST__ = $tab_list_preference?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$tab): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dd <?php if( $tab['field_key'] == ACTION_NAME ): ?><?php $tabName = $tab['field_name']; ?> class="current"<?php endif; ?>><a href="<?php if($tab['url']): ?><?php echo ($tab["url"]); ?><?php else: ?><?php echo U('public/Account/'.$tab['field_key']);?><?php endif; ?>">
        <?php echo ($tab["field_name"]); ?>
        </a></dd><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
    </dl>
    <dl class="nav-left mt10 border-no">
		
        <?php if(is_array($tab_list_security)): ?><?php $i = 0;?><?php $__LIST__ = $tab_list_security?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$tab): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dd <?php if( $tab['field_key'] == ACTION_NAME ): ?><?php $tabName = $tab['field_name']; ?> class="current"<?php endif; ?>><a href="<?php if($tab['url']): ?><?php echo ($tab["url"]); ?><?php else: ?><?php echo U('public/Account/'.$tab['field_key']);?><?php endif; ?>">
        <?php echo ($tab["field_name"]); ?>
        </a></dd><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
    </dl>
</div>
      ...
      <div id="col7" class="main-right">
        <div class="extend minh person-set">
          <div class="set-hd"><h3><?php echo ($tabName); ?></h3></div>
          <!--信息设置项 基本信息-->
          <form action="<?php echo U('public/Account/doSaveProfile');?>" method="post" model-node='account_save'>
            <dl class="form-set">
              <dd>
                <div class="form-tt"><font color="red"> * </font><?php echo L('PUBLIC_LOGIN_EMAIL');?>：</div>
                <div class="form-row"><?php echo ($user_info['email']); ?></div>
              </dd>
              <!--
              <dd>
                <div class="form-tt"><?php echo L('PUBLIC_REAL_NAME');?>：</div>
                <div class="form-row"><?php echo ($user_info['uname']); ?></div>
              </dd>
              -->
              <dd>
                <div class="form-tt"><font color="red"> * </font><?php echo L('PUBLIC_NICKNAME');?>：</div>
                <div class="form-row">
                  <input event-node="uname" event-args="old_name=<?php echo ($user_info['uname']); ?>" name="uname" checkurl="<?php echo U('public/Register/isUnameAvailable');?>" autocomplete="off" type="text" value="<?php echo ($user_info['uname']); ?>" />
                </div>
                <input type="hidden" name="old_name" value="<?php echo ($user_info['uname']); ?>" />
              </dd>
              <dd>
                <div class="form-tt"><font color="red"> * </font><?php echo L('PUBLIC_GENDER');?>：</div>
                <div class="form-row">
                  <label><input name="sex" type="radio" value="1" class="s-ck" <?php if(1 == $user_info['sex']): ?>checked="checked"<?php endif; ?> /><?php echo L('PUBLIC_MALE');?></label>
                  <label><input name="sex" type="radio" value="2" class="s-ck" <?php if(2 == $user_info['sex']): ?>checked="checked"<?php endif; ?> /><?php echo L('PUBLIC_FEMALE');?></label>
                </div>
              </dd>
              <dd>
                <div class="form-tt"><font color="red"> * </font>地区：</div>
                <div class="form-row"><?php echo W('Area',array('curPro'=>$user_info['province'],'curCity'=>$user_info['city'],'area'=>$user_info['area'],'city_names'=>$user_info['location'],'tpl'=>'selectArea'));?></div>
              </dd>
              <!-- <dd>
                <div class="form-tt"><font color="red"> * </font>职业信息：</div>
                <div class="form-row">
                  <font id="userCategory"><?php echo ($user_info['category']); ?></font>&nbsp;&nbsp;
                  <a style="color:#36c" href="javascript:void(0)" onclick="editUserCategory();">修改</a>
                </div>
              </dd> -->
              <!--
              <dd>
                <div class="form-tt"><?php echo L('PUBLIC_LANG_SET');?>：</div>
                <div class="form-row">
                  <?php empty($user_info['lang']) &&  $user_info['lang'] = C('DEFAULT_LANG'); ?>
                  <?php if(is_array($langType)): ?><?php $i = 0;?><?php $__LIST__ = $langType?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$l): ?><?php ++$i;?><?php $mod = ($i % 2 )?><label><input type ='radio' name='lang' value='<?php echo ($l); ?>' <?php if(($user_info["lang"])  ==  $l): ?>checked="checked"<?php endif; ?>> <?php echo ($l); ?></label><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?> 
                </div>
              </dd>
              <?php if(is_array($user_profile_setting[ACTION_NAME]['child'])): ?><?php $i = 0;?><?php $__LIST__ = $user_profile_setting[ACTION_NAME]['child']?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$u_p_s_v): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dd>
                <div class="form-tt"><?php if(($u_p_s_v["required"])  ==  "1"): ?><font color="red"> * </font><?php endif; ?> <?php echo L('PUBLIC_PROFILESET_'.strtoupper($u_p_s_v['field_key']));?>：</div>
                <div class="form-row">
                  <?php echo W('FormInput', array_merge($u_p_s_v, array('type' => $u_p_s_v['form_type'], 'name' => $u_p_s_v['field_key'], 'default_value' => $u_p_s_v['form_default_value'], 'value' => $user_profile[$u_p_s_v['field_id']]['field_data'],'required'=>$u_p_s_v['required'],'field_name'=>$u_p_s_v['field_name'],'canChange'=>0,'editable'=>$u_p_s_v['editable'])));?>
                </div>
              </dd><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
              -->
              <!-- <dd>
                <div class="form-tt">个人标签：</div>
                <div class="form-row">
                <?php echo W('Tag', array('width'=>'400px', 'row_id'=>$mid, 'tpl'=>'tags'));?>
                </div>
              </dd> -->
              <dd>
                <div class="form-tt"> 个人简介：</div>
                <div class="form-row">
                  <textarea class="s-textarea" name="intro"><?php echo ($user_info['intro']); ?></textarea>
                </div>
              </dd>
              <dd>
                <div class="form-tt"></div>
                <div class="form-row">
                  <a event-node="submit_btn" href="#" class="btn-green-big"><?php echo L('PUBLIC_SAVE');?></a>
                </div>
...
var editUserCategory = function(){
    ui.box.load("<?php echo U('public/Account/editUserCategory');?>", '修改身份');
};
</script>
<div class="footer">
