<?php if (!defined('THINK_PATH')) exit();?><div id="selectUploadType" class="chanage">
...
<form id='noflashUpload' model-node="avatar_upload_form" enctype="multipart/form-data" action="<?php echo U('public/Account/doSaveAvatar', array('step'=>'upload'));?>" method="post">
    <dl class="form-avatar">
        <dd>
            ...
            <p>支持JPG,JPGE,PNG格式，大小限制<?php echo (($attach_max_size)?($attach_max_size):0); ?>M以内</p>
        </dd>
        <dd>
            <div model-node="avatar_scan" class="mod-avatar">
                <div class="cut-1"><img event-node="avatar_big" src="<?php echo ($avatar["avatar_big"]); ?>" width="200" /><p>200*200</p></div>
            </div>
        </dd>
    </dl>
</form>
<form model-node="avatar_setting_form" enctype="multipart/form-data" action="<?php echo U('public/Account/doSaveAvatar', array('step'=>'save'));?>" method="post" style="display:none;">
    <dl class="form-avatar">
        ...
        <dd>                                    
            ...
            <a event-node="avatar_save" event-args="tip=<?php echo L('PUBLIC_IMAGE_SAVE_IS');?>？" class="btn-green-small" href="#"><span><?php echo L('PUBLIC_SAVE');?></span></a>
            <a event-node="avatar_reset" class="btn-cancel ml10" href="#"><span><?php echo L('PUBLIC_CANCEL');?></span></a>
        </dd>
    </dl>
</form>
